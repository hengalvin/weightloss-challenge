...
type registerRequest struct {
	...
	InitialWeight int    `json:"initial_weight" binding:"required,gt=10000"`
}
...
func (app *application) registerUser(c *gin.Context) {
	...
	weight := database.Weight{
		UserId: user.Id,
		Weight: user.InitialWeightGr,
	}
	err = app.models.Weight.Insert(&weight)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not insert weight"})
		return
	}
...
func (app *application) login(c *gin.Context) {
	...
	if existingUser == nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid email or password -XX"})
		return
	}
